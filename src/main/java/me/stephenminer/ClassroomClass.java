...
import me.stephenminer.search.LinkSearch;
import me.stephenminer.search.ReflectionSearch;
...
public class ClassroomClass {
    ...
    private Search<?> task;
    public ClassroomClass(String name, WebDriver driver){
        this.name = name;
        this.driver = driver;
    ...
    public void getReflections(){
        enterClass();
        task = new ReflectionSearch(driver);
        task.search();
    }

    public void getLinks(String assignmentName){
        enterClass();
        task = new LinkSearch(driver, assignmentName);
        task.search();
    }

    public void enterClass(){
   //     driver.findElement(By.ByClassName.className("onkcGd Nmpzvc Vx8Sxd")).getText();
        driver.findElement(By.ByXPath.xpath("//a[@aria-label='" + name + "']")).click();
        task = new ReflectionSearch(driver);

    }


    private void getWeeklyQuestions() {
        try {
            Thread.sleep(4000);
    ...
    public Search<?> getTask(){ return task; }
}
